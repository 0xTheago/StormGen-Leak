...
#cap = config.get('capmonster')
#get_balance_resp = httpx.post(f"https://api.capmonster.cloud/getBalance", json={"clientKey": cap}).text
u = open("usernames.txt", 'r', encoding="utf8").read().splitlines()
uu=len(u)
p = open("proxies.txt", 'r', encoding="utf8").read().splitlines()
pp=len(p)
#response_dict = json.loads(get_balance_resp)
#bb = f"{response_dict['balance']:.2f}"
os.system("title stormgen ^| discord token generator")
os.system(f"cls")
colo = '\x1b[38;5;201m'
b=(f"""{colo}
     _                                        
 ___| |_ ___  _ __ _ __ ___   __ _  ___ _ __  
...
#print("{Fore.WHITE}({Fore.LIGHTBLUE_EX}i{Fore.WHITE}) Key Balance: {bb}")
